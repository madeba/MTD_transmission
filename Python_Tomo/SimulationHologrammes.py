...
kappa = 0.01
Radius = 20
DOSSIERDATA = "C:/Users/p1600109/Documents/Recherche/MatlabTomo/SimuHolo/"
...
start_time = time.time()    
TomoSpectrum = fftshift(fftn(Bille))
...
for cpt in range(NB_HOLO):
    ...
    decal = np.array([-k_inc[0], -k_inc[1]]).astype(int)
    TF_Holo_r = rp.decal_TF_holo(TF_Holo, decal, TF_Holo.shape[0])
    HOLO[:, :, cpt] = ifftn(fftshift(TF_Holo_r))*(UBornPitch**2*FMAXHOLO**2)
