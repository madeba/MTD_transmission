...
plt.imshow(abs(UBornCplx[:,:,0])**2,cmap="gray")
plt.show()

del ReUBorn, ImUBorn

start_time = time.time()
f_recon, TF_vol, mask_sum = rp.retropropagation(UBornCplx,nb_holo,fi,fmaxHolo,REwald,lambda_0,1.515,2*dimHolo,dimHolo,pixTheo,2*dimHolo,UBornPitch)
...
plt.imshow(abs(f_recon[:,:,210]),cmap="gray")
plt.show()
...
#     Refr = fftshift(Refraction[:,:,cpt])
#     Abs = fftshift(Absorption[:,:,cpt])
#     # Redon = mask_sum[:,:,cpt]
#     Refr.tofile(fidRef)
#     Abs.tofile(fidAbs) 
...
# fidAbs.close()
# fidRedon.close()
plt.imshow(Absorption[:,:,210],cmap="gray")
plt.show()
plt.imshow(Refraction[:,:,210],cmap="gray")
plt.show()
