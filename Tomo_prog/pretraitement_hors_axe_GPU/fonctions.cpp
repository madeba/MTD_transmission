...
void calcPhase_mpi_pi_atan2(std::vector<std::complex <double> > const &cplxField, vector<double> &phaseMod2pi)///calcul phase -PI-PI
{
for(unsigned int cpt=0;cpt<cplxField.size();cpt++)
...
void   decal2DCplxGen2(vector<std::complex<double>> const &entree,vector<std::complex<double>>  &result, Var2D   &decalGen){
...
int coordSpec(vector<std::complex<double>> const &TF_UBorn, vector<double> &TF_champMod,Var2D NMAX)
 {
    int cpt_max=0;
...
void coupeCplx(vector<std::complex<double>> const &src, vector<std::complex<double>> &dest, Var2D dim_src, Var2D dim_dest, Var2D coin, size_t NumAngle)
{
...
//fonction très particulière à n'utiliser qu'après un fftshift, typiquement sur un spectre centre en repère informatique
void coupe2D_I_to_H3D(vector<std::complex<double>> const &src2D, vector<std::complex<double>> &dest3D,Var2D dim_dest2D, unsigned short int numAngle)
{
...
void holo2TF_UBorn2_shift(vector<double>  &holo1,vector<std::complex<double>> &TF_UBornTot,Var2D dimROI,Var2D dim2DHA,Var2D coinHA_shift, size_t NbAngleOk, vector<double> const &tukeyHolo,FFTW_init  &param_fftw2DHolo)
{
    size_t NbPixROI2d=holo1.size();
    vector<std::complex<double>> TF_Holo(NbPixROI2d);
...
void holo2TF_UBorn2_shift_r2c(vector<double>  &holo1,vector<std::complex<double>> &TF_UBornTot,Var2D dimROI,Var2D dim2DHA,Var2D coinHA_shift, size_t NbAngleOk, vector<double> const &tukeyHolo,FFTW_init  &param_fftw2D_r2c_Holo)
{
    size_t NbPixROI2d=holo1.size();
    vector<std::complex<double>> TF_Holo(NbPixROI2d);
    for(size_t pixel=0; pixel<NbPixROI2d; pixel++)
...
void holo2TF_UBorn2(vector<double>  &holo1,vector<std::complex<double>> &TF_UBornTot,Var2D dimROI,Var2D dim2DHA,Var2D coinHA, size_t NbAngleOk, vector<double> const &tukeyHolo,FFTW_init  &param_fftw2DHolo)
{
    size_t NbPixROI2d=holo1.size();
    vector<std::complex<double>> TF_Holo(NbPixROI2d);
    for(size_t pixel=0; pixel<NbPixROI2d; pixel++)
...
void holo2TF_UBorn(vector<double> holo1, vector<std::complex<double>> &TF_UBornTot,Var2D dimROI, Var2D dim2DHA, Var2D coinHA, size_t NumAngle, vector<double> tukey_holo, fftw_complex *in,fftw_complex *out,fftw_plan p_forward_holo)
{
    ///--------------Init FFTW-------------------------------------------------
    ...
    vector<std::complex<double>> TF_Holo(NbPixROI2d);
    vector<std::complex<double>> TFHoloCentre(NbPixROI2d);
...
void holo2TF_UBorn_old(vector<double> holo1, vector<std::complex<double>> &TF_UBornTot,Var2D dimROI, Var2D dim2DHA, Var2D coinHA, size_t NumAngle, vector<double> tukey_holo)
{
     ///--------------Init FFTW-------------------------------------------------
    size_t NbPix2dROI=holo1.size();
    size_t dimx=sqrt(NbPix2dROI);
...
void calc_Uborn2(vector<std::complex<double>> const &TF_UBorn,vector<std::complex<double>> &UBorn,Var2D dim2DHA,Var2D PosSpec,FFTW_init &param_c2c)
{
    Var2D recalUBorn={-PosSpec.x,-PosSpec.y},DecalU_Born={dim2DHA.x/2,dim2DHA.y/2};
    size_t NbPixUBorn=dim2DHA.x*dim2DHA.y;

    vector<std::complex<double>> TF_UBorn_I(NbPixUBorn);

    decal2DCplxGen2(TF_UBorn,TF_UBorn_I,recalUBorn);

    vector<std::complex<double>> UBorn_I(NbPixUBorn);
