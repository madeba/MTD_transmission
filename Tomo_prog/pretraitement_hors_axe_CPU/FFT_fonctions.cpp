...
std::vector<T> fftshift2DGPT(const std::vector<T>& entree)
{
    ...
    const size_t dim = static_cast<size_t>(std::sqrt(N));
    if (dim * dim != N) throw std::runtime_error("fftshift2D: input size is not a perfect square");

    std::vector<T> result(N);

    // décalage = floor(dim/2), marche aussi pour impair
    size_t decal = dim / 2;

    #pragma omp parallel for collapse(2) // facultatif si OpenMP activé
    for (size_t y = 0; y < dim; ++y)
    {
        for (size_t x = 0; x < dim; ++x)
        {
            // Coordonnées shiftées avec wrap-around modulo dim
            size_t xs = (x + decal) % dim;
            size_t ys = (y + decal) % dim;

            result[ys * dim + xs] = entree[y * dim + x];
        }
    }
...
///crop the hologramm spectrum to complex field pectrum and add it to a stack containing all the fields.
