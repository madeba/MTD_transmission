...
//init statique nbthreads
void FFTW_init::init_threads(size_t nbThread)
{
    ...
    fftw_plan_with_nthreads(nbThread);
}

///init fftw for FFT3D
FFTW_init::FFTW_init(Point3D dim,size_t nbThread)
{
    size_t nbPix=dim.x*dim.y*dim.z;
...
FFTW_init::FFTW_init(vector<double>const &entree, string str_geometry,size_t nbThread)
{
  ...
  //  in=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nbPix);
    if(str_geometry=="r2c"){
    size_t nbPixCplx=dim2D.x*(dim2D.y/2+1);
    size_t nbPix = dim2D.x*dim2D.y;
...
FFTW_init::FFTW_init(vector<complex<double>>const &entree,size_t nbThread)
{
   ...
   // fftw_plan_with_nthreads(m_Nthread);
    in=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nbPix);
    out=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nbPix);
    p_forward_OUT=fftw_plan_dft_2d(dim, dim, in, out,FFTW_FORWARD , FFTW_PATIENT);
...
FFTW_init::~FFTW_init()
{
   // cout<<"destructeur"<<endl;
    ...
    if(in_double) fftw_free(in_double);

    if(p_forward_OUT) fftw_destroy_plan(p_forward_OUT);
    if(p_backward_OUT) fftw_destroy_plan(p_backward_OUT);
    if(p_forward_IN) fftw_destroy_plan(p_forward_IN);
    if(p_backward_IN) fftw_destroy_plan(p_backward_IN);

    fftw_cleanup_threads(); // important si threads utilisés
}
