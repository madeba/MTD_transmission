...
#include "vecteur.h"
#include "deroulement_volkov4_symetrie.h"
#include <chrono>
#define M_2PI 2*M_PI
using namespace std;
...
Symetrise_OddX(gradx_IntM, AS_gradx_IntM);//odd parity for grad_X
Symetrise_EvenY(grady_IntM, AS_grady_IntM);//even parity for grad_Y
...
///main function, all calculation with FFT
void deroul_volkov4(vector<double> const &phase_enroul,vector<double> &phase_deroul,vector<vecteur>  &kvect_shift,  FFTW_init &param_c2c)
{
  complex<double> I(0,1);
  unsigned int nbPix=phase_enroul.size();
  Var2D dimROI={sqrt(nbPix),sqrt(nbPix)};
...
void gradient_fft4(vector<double> const &entree, vector<complex<double>> &gradx, vector<complex<double>> &grady, vector<vecteur> &kvect_shift, FFTW_init &param_c2c)
{
     complex<double> I(0,1);
...
void gradient_fft4(vector<complex<double>> const&entree, vector<complex<double>> &gradx, vector<complex<double>> &grady,vector<vecteur>  &kvect_shift, FFTW_init &param_c2c)
{
  complex<double> I(0,1);
  unsigned int nbPix=entree.size();
  unsigned int dim=sqrt(nbPix);
...
vector<double> Symetrise_OddX(vector<double> const &monImg, vector<double> &monImgSymetricX){
...
///applied optics 51-23. Bon 2012 : even symetry for X derivative before fft integration to avoid artefact. only X direction as a return value
vector<double> Symetrise_EvenY(vector<double> const &monImg, vector<double> &monImgSymetricY){
    size_t cpt_orig,cpt0_final,cpt1_final,cpt2_final,cpt3_final;
    size_t dim=sqrt(monImg.size());
    for(size_t x=0;x<dim;x++)
    for(size_t y=0;y<dim;y++){
