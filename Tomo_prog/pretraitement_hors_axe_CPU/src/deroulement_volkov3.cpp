...
///main function : calculate gradient of wrapped phase, Z and integrate, to find integer field
...
SAV_Tiff2D(gradx_enroul_fft, "Re","/home/mat/tomo_test/gradx_enroul_volkov3.tiff", 1);

  for(size_t cpt=0;cpt<nbPix;cpt++){
    Z[cpt].real(cos(phase_enroul[cpt]));
    Z[cpt].imag(sin(phase_enroul[cpt]));
  }
  SAV_Tiff2D(Z, "Re","/home/mat/tomo_test/Z_volkov3.tiff", 1);
...
void gradient_fft3(vector<double> const &entree, vector<complex<double>> &gradx, vector<complex<double>> &grady, vector<vecteur> &kvect_shift, FFTW_init &param_c2c)
{
    ...
    //limiter le spectre. on décale au centre pour faciliter le filtrage
    ///limit the spectrum, the spectrum is shifted to facilitate the operation
...
//     SAV_Tiff2D(spectre_shift, "Re","/home/mat/tomo_test/spectre_shift_volkov3.tiff", 1);
   // spectre=fftshift2D(spectre_shift);
    SAV_Tiff2D(spectre, "Re","/home/mat/tomo_test/spectre_volkov3.tiff", 1);
//calcul du gradient par fft
...
///commented because already defined in file FFT_fonctions.cpp
/*
