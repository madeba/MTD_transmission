...
void phaseUnwrapping_Mat(Var2D taille, vector<double> WrappedImage_herraez, double *UnwrappedImage)
{
    int image_size=taille.x*taille.y;
    double * WrappedImage =new double[image_size];
    for(int cpt=0;cpt<image_size;cpt++)
      WrappedImage[cpt]=WrappedImage_herraez[cpt];

    //WrappedImage=WrappedImage_herraez.data();
