...
manip::manip()
{
   ...
   cout<<"taille_theo_pixeltomo :"<<tailleTheoPixelTomo<<endl;
    ...
    kappa_Max=extract_val("KAPPA_MAX",fic_cfg_recon);
    kappa_Min=extract_val("KAPPA_MIN",fic_cfg_recon);
    delta_nMax=extract_val("DELTA_NMAX",fic_cfg_recon);
    delta_nMin=extract_val("DELTA_NMIN",fic_cfg_recon);
    nbIterGPS=extract_val("NB_ITER_GPS",fic_cfg_recon);
    if(b_Born==true)
        cout<<"RYTOV=0"<<endl;
    else
        cout<<"RYTOV=1"<<endl;
